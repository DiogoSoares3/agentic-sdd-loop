...
CONT='Continue the SDD build loop from where it left off. Do NOT re-plan and do NOT rewrite the profile or baselines. For each todo issue in docs/phases/phase-1/backlog.md, one at a time: YOU create and check out the issue/<id>-<slug> branch off develop, then dispatch a fresh sdd-issue-worker onto it — BDD outer test first (prove RED, commit test alone), run /tdd inner loop only when the flag is required, make green (python3 -m pytest -q). The worker NEVER merges and never leaves its branch; it returns ready-to-land. Then dispatch a bounded sdd-merge-resolver to land that branch onto develop (rebase, full suite, merge — auto-merge). Keep the SDD-CURSOR current. Continuation mode is auto — do not ask.'
