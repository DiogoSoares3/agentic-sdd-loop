...
# registers the plugin's three hooks (plus a probe that logs every SessionStart source).
...
# settings.json — registers the plugin's 3 hooks headlessly + a SessionStart source probe.
cat > "$WORK/settings.json" <<EOF
{
  "hooks": {
    ...
    "PreToolUse": [
      { "matcher": "Edit|Write",
        "hooks": [ { "type": "command", "command": "bash '$PLUGIN_HOOKS/sdd-enforce-test-first.sh'" } ] }
    ],
    "SubagentStop": [
      { "matcher": "*",
