...
#   8. worker    — a dispatched sdd-issue-worker INVOKES /bdd + /tdd, and the TDD flag still gates the inner loop
#
...
# 8 is a two-level run (main agent dispatches a real subagent) — it is the slowest; give it headroom.
#
# Prereqs: bubblewrap (`bwrap`) · a logged-in claude CLI (`~/.claude/.credentials.json`) · network · pytest.
# Cost: eight Haiku runs (gates three turns; testfirst and bdd two; worker three two-level dispatches — the
# slowest by far, a real subagent per context). Nothing persists outside WORK mktemps.
#
#   bash run-bwrap.sh                 # all eight
#   SCENARIO=land bash run-bwrap.sh   # one of: gates | land | guard | red | tdd | testfirst | bdd | worker
...
declare -A RC
for s in ${SCENARIO:-gates land guard red tdd testfirst bdd worker}; do
