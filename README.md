...
|---|---|---|
| **1. Core methodology** | the invariant SDD+TDD loop — state machine, gates, escalation, change control, dispatcher | `skills/sdd/SKILL.md` + `skills/sdd/dispatcher.md` |
...
- **PLAN** cuts the next epic from the root PRD in **dependency order**, with **MoSCoW priority as the
  must-first tiebreak**, and writes a thin **phase PRD** (`docs/phases/phase-N/prd.md`) — a projection that
  references the baselines by ID/seam, never a second full PRD.
...
- **BUILD** dispatches issues **one at a time by a fresh agent** via the per-issue **dispatcher**
  ...
  flag, default on) until both are green. Integrity guards (immutable scenario, prove-RED,
...
to `main`. **Escalation:** when a decision the baselines don't answer arises (structural/critical), the
loop **stops and runs `/grill-me`** with the engineer (→ ADR) or stakeholder (→ PRD amendment), then
resumes — the spec grows, controlled.

The loop is **finite** (one iteration = one issue; a phase is bounded by its backlog size). The iteration
**contract is host-agnostic** — any mechanism that re-enters the dispatcher works: builtin `/loop`, the
`auto` supervisor, or a plain re-invocation of `/sdd`. At the context gate it continues per the **handoff
mode**: `manual` (a human starts a clean session) or `auto` (a **flat supervisor** sequentially respawns
fresh **worker subagents** — workers hold all context and self-checkpoint, the near-empty supervisor only
respawns them; no human, requires subagent support). A **mid-work cut (~40%)** writes a `/handoff`; a
**clean boundary** (phase drained / project done) needs none — files fully describe it. State lives
entirely in files, so any fresh context (subagent, new session, harness compaction) resumes from exact
position. With **`auto-merge` + `auto` handoff** the loop runs unattended, stopping only for a genuine
PRD/ARCHITECTURE decision.
...
| **Fresh-agent mode** | `reprime` | `reprime` \| `subagent` (worktree per issue) |
| **Handoff mode** | `manual` | `manual` \| `auto` (flat supervisor; needs subagents) |
| **Backlog review** | `auto` | `auto` \| `confirm` (pause after `/to-issues` for human sign-off) |
| **Integrity enforcement** | `prose+git` | `+verifier` (agent) \| `+hook` (block test-path edits on green) |
...
## Loop
- Fresh-agent mode: reprime
- Handoff mode: auto
- Backlog review: auto
- Integrity enforcement: prose+git +verifier
